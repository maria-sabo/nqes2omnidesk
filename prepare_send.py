...
def prepare_and_send(email, api_token, subdomain):
    ...
    Происходит цикл по записям, полученным в результате запроса к БД:
        Из каждой записи берутся
        имя сотрудника, которому выпускается УНЭП,
        дата заявки,
        email и номер телефона из заявки,
        компания сотрудника.
        ...
        Если сообщение успешно отправлено, то дата отправленной заявки перезаписывается в файл.
